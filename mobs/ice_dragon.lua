...
minetest.register_entity("draconis:ice_eyes", {
    ...
    textures = {"draconis_ice_eyes_blue.png"},
    is_visible = true,
            ...
            if parent.status ~= "sleeping" then
                ...
                if parent.age < 25 then
                    self.object:set_properties(
                        {textures = {"draconis_ice_eyes_child_"..self.color..".png"}})
                else
                    self.object:set_properties(
                        {textures = {"draconis_ice_eyes_"..self.color..".png"}})
                end
        ...
        and self.logic_state == "flying")) then
            if self.order == "stand" then
                if not self.isonground then
                    draconis.hq_land_and_wander(self, 20)
                else
        ...
        and self.stance ~= "passive" then
            ...
            and not mob_core.shared_owner(self, target) then
                if self.logic_state == "landed" then
                    ...
                    draconis.hq_aerial_wander(self, 9)
                end
            end
        end
                ...
                for _, mob in ipairs(draconis.mobkit_mobs) do
                    ...
                    and not mob_core.shared_owner(self, target) then
                        if self.logic_state == "landed" then
                            ...
                            draconis.hq_aerial_wander(self, 5)
                        end
                    end
                end
            end
        ...
        and player then
            ...
            and player:get_player_name() ~= self.owner) then
                ...
                elseif self.logic_state == "flying" then
                    ...
                    draconis.hq_aerial_wander(self, 3)
                end
            end
        end
...
draconis.register_dragon("ice", {
    colors = {"light_blue", "sapphire", "slate", "white", "silver"},
